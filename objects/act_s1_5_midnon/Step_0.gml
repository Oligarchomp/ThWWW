	...
	if(spell_wait == 0)
	{
		...
		if (obj_boss.in_position)
		{
			if((shoot_ref_time2 + step) % shoot_wait == shoot_wait / 2)
			{
				...
				shoot_ring(DAN_BUBBLE,4,ring_nbr ,obj_boss.x,obj_boss.y,angle_ref,5,sfx_shot1,6);
				shoot_ring(DAN_BALL,4,ring_nbr ,obj_boss.x,obj_boss.y,angle2,5,sfx_shot1,6);
		...
		else
		{
			...
			{	
				shoot_arc_row(DAN_BALL,6,aim_arc,aim_row,obj_boss.x,obj_boss.y,999,30,aim_spd_min,aim_spd_max,sfx_shot3,5)
			}
		}

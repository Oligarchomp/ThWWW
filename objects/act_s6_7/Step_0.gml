...
if(global.gp_active)
{
	...
	switch(state_time)
	{
		case 0:
		case 260:
			var inst = create_enemy(EN_RED,-20,110,f_life,1,4,0);
			inst.item_nbr = 20;
			var inst = create_enemy(EN_RED,room_width + 20,110,f_life,1,4,180);
			inst.item_nbr = 20;
		break;
	}
				...
				if(state_time < shoot_lenght)
				{
					if(state_time % bubble_wait == 0)
					{
						for(var i = 0; i < 360; i += 360 / bubble_ring)
						{
							...
							var inst = shoot(DAN_BUBBLE,6,x,y,ang + i,bubble_spd_shoot,sfx_redirect1,7);
							inst.dan_off = i;
						}
					}
				}
			...
			case 0:
				if(state_time < bubble_lenght)
				{
					if(state_time % ball_wait == ball_wait - 1)
					{
						...
						var inst = shoot(DAN_BALL,6,x,y,ang,0,sfx_shot2,4);
					}
				}
				else
				{
